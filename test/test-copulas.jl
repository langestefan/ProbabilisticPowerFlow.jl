@testitem "CopulaDependence construction and validation" tags = [:unit, :fast] begin
    import Copulas
    import ProbabilisticPowerFlow as PPF
    using Distributions: Normal

    dep = CopulaDependence(Copulas.ClaytonCopula(2, 2.0))
    @test PPF.dependence_dim(dep) == 2
    ...
    @test UncertaintyModel(vars, assigns, dep) isa UncertaintyModel
    # dimension mismatch is caught at model construction
    @test_throws ArgumentError UncertaintyModel(
        vars,
        assigns,
        CopulaDependence(Copulas.ClaytonCopula(3, 2.0)),
    )

    # wrapping something that is not a copula throws through both seam functions
    bad = CopulaDependence([1.0 0.0; 0.0 1.0])
    @test_throws ArgumentError PPF.dependence_dim(bad)
    @test_throws ArgumentError PPF.to_dependent!(zeros(2), bad, [0.5, 0.5])
end

@testitem "CopulaDependence is deterministic in u" tags = [:unit, :fast] begin
    import Copulas
    ...
    mk() = UncertaintyModel(
        ...
        CopulaDependence(Copulas.GumbelCopula(2, 1.7)),
    )
    m1 = mk()
    u = [0.31, 0.77]
    @test to_physical(m1, u) == to_physical(m1, u)
    # a fresh identical wrapper gives the same map
    @test to_physical(m1, u) == to_physical(mk(), u)
end

@testitem "CopulaDependence clamps extreme inputs" tags = [:unit, :fast] begin
    import Copulas
    import ProbabilisticPowerFlow as PPF
...
@testitem "Gaussian family matches the built-in copula pointwise" tags = [:unit] setup =
    ...
    Rg = [1.0 0.6; 0.6 1.0]
    wrapped = CopulaDependence(Copulas.GaussianCopula(Rg))
...
@testitem "MC end-to-end with a Clayton copula on case5" tags = [:integration] begin
    ...
    model = UncertaintyModel(
        ...
        CopulaDependence(Copulas.ClaytonCopula(2, 2.0)),
    )
